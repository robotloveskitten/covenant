  ...
  updateSelectedDisplay(name) {
    if (name) {
      this.selectedTarget.innerHTML = `
        <span class="inline-flex items-center gap-2" style="white-space: nowrap;">
          <span class="inline-flex w-6 h-6 rounded-full bg-base-300 items-center justify-center text-xs font-medium flex-none">
            ${name.charAt(0).toUpperCase()}
          </span>
          <span class="text-sm">${this.escapeHtml(name)}</span>
          <button type="button" data-action="click->combobox#clear" class="inline-flex flex-none text-base-content/40 hover:text-base-content">
